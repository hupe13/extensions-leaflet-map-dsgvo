...
define( 'LEAFEXT_DSGVO_PLUGIN_VERSION', '1.0' );

// for translating a plugin
function leafext_dsgvo_textdomain() {
	if ( get_locale() === 'de_DE' ) {
		load_plugin_textdomain( 'dsgvo-leaflet-map-github', false, LEAFEXT_DSGVO_PLUGIN_NAME . '/lang/' );
...
function leafext_add_action_dsgvo_links( $actions ) {
	$actions[] = '<a href="' . esc_url( admin_url( 'admin.php' ) . '?page=' . LEAFEXT_DSGVO_PLUGIN_NAME ) . '">' . esc_html__( 'Settings' ) . '</a>';
	return $actions;
}
